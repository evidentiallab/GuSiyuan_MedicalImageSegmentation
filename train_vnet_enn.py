...
from nets.vnet_enn import VNetENN
...
train_session_dir = os.path.join(args.save_dir, f"vnet_enn_train_session_{formatted_time}")
os.makedirs(train_session_dir, exist_ok=True)

data_dir = args.data_dir
pet_dir = os.path.join(data_dir, 'SUV_cropped')
ct_dir = os.path.join(data_dir, 'CTres_cropped')
mask_dir = os.path.join(data_dir, 'SEG_cropped')
...
model = VNetENN(model_params).to(device)


trained_model_path = "./models/pretrained_model.pth"  # path to the pretrained VNet model
model.load_pretrained_vnet(trained_model_path)
