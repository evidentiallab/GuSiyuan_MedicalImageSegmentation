...
from nets.attentionunet_enn import AttentionUNetENN
...
train_session_dir = os.path.join(args.save_dir, f"attentionunet_enn_train_session_{formatted_time}")
...
model = AttentionUNetENN(model_params).to(device)


trained_model_path = "./models/pretrained_model.pth"  # path to the pretrained UNet model
model.load_pretrained_attentionunet(trained_model_path)
